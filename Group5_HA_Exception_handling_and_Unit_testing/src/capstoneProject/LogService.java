...
public class LogService {
	...
	private void createLogDirectory() throws LogFileCreationException {
		try {
			...
			throw new LogFileCreationException(e.getMessage());
		}
	}
				...
				for (File logFile : logFiles) {
					if (pattern.matcher(logFile.getName()).find()) {
						try {
							...
							throw new LogFileDoesNotExistException("Failed to delete log file: " + logFile.getName());
						}
					}
				}
			} else {
				...
				for (File logFile : logFiles) {
					if (pattern.matcher(logFile.getName()).find()) {
						try {
							...
							throw new LogFileCreationException("Failed to move log file: " + logFile.getName());
						}
					}
				}
			}
	...
	public void archiveLogFile(String logFileName) throws LogFileArchiveException, LogFileCreationException {
		...
		try (ZipOutputStream zipOutputStream = new ZipOutputStream(new FileOutputStream(zipFileName))) {
			...
			throw new LogFileCreationException("Failed to create ZIP file: " + e.getMessage());
		}
	}
		...
		try (DirectoryStream<Path> stream = Files.newDirectoryStream(Paths.get(logDirectory),
			...
			for (Path entry : stream) {
				...
				try (BufferedReader reader = new BufferedReader(
					...
					throw new LogFileReadException(e.getMessage());
				}
			}
		} catch (IOException e) {
			throw new LogFileDoesNotExistException(e.getMessage());
		}
